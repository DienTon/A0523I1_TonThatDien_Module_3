...
<div class="row">
    ...
    <form method="get" class="col-8" action="/UserServlet">
...
<table class="table">
    ...
    <c:forEach items="${list}" var="c" varStatus="loop">
        <tr>
            ...
            <td>${c.email}</td>
            <td>${c.country}</td>
            <td><a href="/UserServlet?action=edit&id=${c.getId()}"><button>Edit</button></a></td>
            <td><a href="/UserServlet?action=delete&id=${c.getId()}"><button>Delete</button></a></td>
        </tr>
    </c:forEach>
</table>
</body>
